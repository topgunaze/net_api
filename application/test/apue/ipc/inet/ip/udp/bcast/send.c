...
main(int argc, char * argv[])
{
...
#endif
	int val = 1;
	ret = setsockopt(sfd, SOL_SOCKET, SO_BROADCAST, &val, sizeof(val));
	if(ret < 0){
		perror("setsockopt()");
		exit(-1);
	}
	...
	recvaddr.sin_port = htons(RECVPORT);
	//inet_pton(PF_INET, "192.168.11.255", &recvaddr.sin_addr);
	inet_pton(PF_INET, "255.255.255.255", &recvaddr.sin_addr);
	while(1){
