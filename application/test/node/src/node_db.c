...
static sqlite3 *p_node_onu_db = NULL;
...
#define NODE_DB_LOCK  do{sem_wait(&node_onu_db_sem);}while(0)
#define NODE_DB_UNLOCK  do{sem_post(&node_onu_db_sem);}while(0)


static int
node_onu_db_onu_auth_sn_pwd_tbl_aging(
                ...
                node_loid_user      *p_loid_user)
{
    ...
    if(!p_loid_user_str)
        memset((char *)p_loid_user->arr, 0, sizeof(node_loid_user));
    else
        strncpy((char *)p_loid_user->arr, (const char*)p_loid_user_str, sizeof(node_loid_user));
...
node_loid_user_to_str(
                const node_loid_user *p_loid_user,
                char *p_loid_user_str,
                const uint32_t loid_user_str_size)
{
    if(!p_loid_user || !p_loid_user_str || loid_user_str_size <= sizeof(node_loid_user))
        return -1;
...
static int
node_loid_co_to_str(
                const node_loid_co *p_loid_co,
                char *p_loid_co_str,
                const uint32_t loid_co_str_size)
{
    if(!p_loid_co || !p_loid_co_str || loid_co_str_size <= sizeof(node_loid_co))
        return -1;
...
static int
node_onu_db_onu_black_tbl_aging(
...
int
node_onu_db_onu_black_tbl_should_silence(
                const uint8_t ni,
                const char *p_serial_number)
{
    int ret;
    ...
    sysinfo(&sys_info);
    node_onu_db_onu_black_tbl_aging(sys_info.uptime);

    node_sn_to_str(p_serial_number, sn_str, sizeof(sn_str));
    snprintf(cmd_str, sizeof(cmd_str),
    ...
    EXIT_ON_SQLITE_UNLOCK(p_node_onu_db, ret, SQLITE_OK, cmd_str, FALSE);
                ...
                const uint32_t sec_duration)    //0: no-aging
{
    ...
    ret = sqlite3_exec(p_node_onu_db, "begin;", 0, 0, 0);
    EXIT_ON_SQLITE_UNLOCK(p_node_onu_db, ret, SQLITE_OK, sql, ret);

    do
    {
        ret = sqlite3_prepare_v2(p_node_onu_db, sql, strlen(sql), &stmt, 0);
        BREAK_ON_SQLITE(p_node_onu_db, ret, SQLITE_OK, sql, ret);
        ...
        BREAK_ON_SQLITE(p_node_onu_db, ret, SQLITE_DONE, sql, ret);
...
int
node_onu_db_onu_black_tbl_del_by_sn(
                const uint32_t ni,
                const char *p_serial_number)
{
    int ret;
...
int
node_onu_db_onu_black_tbl_del_by_phy_uint(
...
int
node_onu_db_onu_black_tbl_clear_phy_uint(
                const uint32_t phy_uint)
{
    int ret;
    char cmd_str[MAX_SQLITE_CMD_STR_SIZE];
...
int
node_onu_db_onu_black_tbl_update_by_phy_uint(
                const uint32_t phy_uint,
...
static int
node_create_onu_black_tbl(void)
{
    int ret;

    ret = sqlite3_exec(p_node_onu_db, "drop table if exists onu_black_tbl", 0, 0, 0);
    EXIT_ON_SQLITE(p_node_onu_db, ret, SQLITE_OK, "drop onu_black_tbl failed", ret);

    ret = sqlite3_exec(p_node_onu_db, "create table onu_black_tbl("
                                            "ni int8 not null, "
                                            ...
                                            NULL, NULL, NULL);
    EXIT_ON_SQLITE(p_node_onu_db, ret, SQLITE_OK, "create onu_black_tbl failed", ret);
...
int
node_onu_db_onu_phy_info_tbl_aging_duration_set(
...
int
node_onu_db_onu_phy_info_tbl_aging_duration_get(
...
int
node_onu_db_onu_phy_info_tbl_get_free_phy_id(
                const uint8_t ni,
                uint16_t *p_phy_id)
{
    int ret;
    ...
    ret = sqlite3_prepare_v2(p_node_onu_db, cmd_str, -1, &stmt, 0);
    EXIT_ON_SQLITE_UNLOCK(p_node_onu_db, ret, SQLITE_OK, cmd_str, ret);
...
int
node_onu_db_onu_phy_info_tbl_add(
                const char *p_serial_number,
...
int
node_onu_db_onu_phy_info_tbl_set_password(
                const uint32_t phy_uint,
                const char *p_password)
{
    int ret;
    ...
    node_pwd_to_str(p_password, pwd_str, MAX_PWD_STR_SIZE);
    snprintf(cmd_str, sizeof(cmd_str),
...
int
node_onu_db_onu_phy_info_tbl_set_loid(
                const uint32_t phy_uint,
                const node_loid_user *p_loid_user,
                const node_loid_co *p_loid_co)
{
    ...
    node_loid_user_to_str(p_loid_user, loid_user_str, MAX_LOID_USER_STR_SIZE);
    node_loid_co_to_str(p_loid_co, loid_co_str, MAX_LOID_CO_STR_SIZE);
    snprintf(cmd_str, sizeof(cmd_str),
...
int
node_onu_db_onu_phy_info_tbl_set_eqd(
                const uint32_t phy_uint,
                const uint32_t eqd)
{
    int ret;
...
int
node_onu_db_onu_phy_info_tbl_set_attr(
                const uint32_t phy_uint,
                const node_onu_attr *p_onu_attr)
{
    int ret;
    sqlite3_stmt *stmt;
    char cmd_str[MAX_SQLITE_CMD_STR_SIZE];
    ...
    do
    {
        ret = sqlite3_prepare_v2(p_node_onu_db, cmd_str, len, &stmt, 0);
        BREAK_ON_SQLITE(p_node_onu_db, ret, SQLITE_OK, cmd_str, ret);

        ret = sqlite3_reset(stmt);
        BREAK_ON_SQLITE(p_node_onu_db, ret, SQLITE_OK, cmd_str, ret);

        ret = sqlite3_bind_blob(stmt, 1, p_onu_attr, sizeof(node_onu_attr), NULL);
        BREAK_ON_SQLITE(p_node_onu_db, ret, SQLITE_OK, cmd_str, ret);

        ret = sqlite3_step( stmt );
        BREAK_ON_SQLITE(p_node_onu_db, ret, SQLITE_DONE, cmd_str, ret);
...
int
node_onu_db_onu_phy_info_tbl_set_status(
                const uint32_t phy_uint,
                const uint32_t status)
{
    int ret;
...
int
node_onu_db_onu_phy_info_tbl_set_autofind(
                const uint32_t phy_uint,
                const uint8_t autofind_flag)
{
    int ret;
...
int
node_onu_db_onu_phy_info_tbl_get_status(
                const uint8_t ni,
...
int
node_onu_db_onu_phy_info_tbl_get_phy_uint(
                const uint8_t ni,
                const char *p_serial_number,
                uint32_t *p_phy_uint,
                uint32_t *p_status)
...
int
node_onu_db_onu_phy_info_tbl_get_valid_phy_uint(
                const uint8_t ni,
                const char *p_serial_number,
                uint32_t *p_phy_uint)
{
...
int
node_onu_db_onu_phy_info_tbl_get_first_by_status(
                const uint32_t status,
                ...
                node_loid_co *p_loid_co*/)
{
    ...
    {
        *p_phy_uint = sqlite3_column_int(stmt, ONU_PHY_INFO_TBL_STMT_IDX_PHY_UINT);
        //node_str_to_sn(sqlite3_column_text(stmt, ONU_PHY_INFO_TBL_STMT_IDX_SN), p_serial_number);
        //node_str_to_pwd(sqlite3_column_text(stmt, ONU_PHY_INFO_TBL_STMT_IDX_PWD), p_password);
        //node_str_to_loid_user(sqlite3_column_text(stmt, ONU_PHY_INFO_TBL_STMT_IDX_LOID_USER), p_loid_user);
        //node_str_to_loid_co(sqlite3_column_text(stmt, ONU_PHY_INFO_TBL_STMT_IDX_LOID_CO), p_loid_co);
        sqlite3_finalize(stmt);
...
int
node_onu_db_onu_phy_info_tbl_get_first_by_pwd(
                const char *p_password,
                const uint32_t phy_uint_end,
                uint32_t *p_phy_uint,
                char *p_serial_number)
...
int
node_onu_db_onu_phy_info_tbl_get_first_by_loid(
                const node_loid_user *p_loid_user,
                const uint32_t phy_uint_end,
                uint32_t *p_phy_uint,
                char *p_serial_number)
...
int
node_onu_db_onu_phy_info_tbl_get_first_expire(
                uint32_t *p_phy_uint)
{
    int ret;
    char cmd_str[MAX_SQLITE_CMD_STR_SIZE];
...
int
node_onu_db_onu_phy_info_tbl_get_first_deleting(
                uint32_t *p_phy_uint)
{
    int ret;
    char cmd_str[MAX_SQLITE_CMD_STR_SIZE];
...
int
node_onu_db_onu_phy_info_tbl_get_first_autofind(
                uint32_t *p_phy_uint,
...
int
node_onu_db_onu_phy_info_tbl_get_sn_pwd(
                const uint32_t phy_uint,
                char *p_serial_number,
                char *p_password)
{
...
int
node_onu_db_onu_phy_info_tbl_get(
                const uint32_t phy_uint,
                char *p_serial_number,
                char *p_password,
                node_loid_user *p_loid_user,
                ...
                node_onu_attr *p_onu_attr)
{
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        *p_eqd = sqlite3_column_int(stmt, ONU_PHY_INFO_TBL_STMT_IDX_EQD);
        ...
        if(blob_size == sizeof(node_onu_attr))
            memcpy(p_onu_attr, sqlite3_column_blob(stmt, ONU_PHY_INFO_TBL_STMT_IDX_ATTR), sizeof(node_onu_attr));
...
int
node_onu_db_onu_phy_info_tbl_del(
                const uint32_t phy_uint)
{
    int ret;
    char cmd_str[MAX_SQLITE_CMD_STR_SIZE];
...
static int
node_create_onu_phy_info_tbl(void)
{
    int ret;
    char * p_errmsg = NULL;

    ret = sqlite3_exec(p_node_onu_db, "drop table if exists onu_phy_info_tbl", 0, 0, 0);
    EXIT_ON_SQLITE(p_node_onu_db, ret, SQLITE_OK, "drop onu_phy_info_tbl failed", ret);

    ret = sqlite3_exec(p_node_onu_db, "create table "
                                            "onu_phy_info_tbl("
                                                "phy_uint int unique, "
                                                "status int,"
                                                "sn text not null COLLATE NOCASE, "
                                            ...
                                            NULL, NULL, &p_errmsg );
    EXIT_ON_SQLITE(p_node_onu_db, ret, SQLITE_OK, "create onu_phy_info_tbl failed", ret);
                ...
                const char *p_serial_number)
{
    ...
    ret = node_onu_db_onu_phy_info_tbl_get_valid_phy_uint(ni, p_serial_number, &phy_uint);
    if(ret)
        return TRUE;

    ret = node_onu_db_onu_auth_info_tbl_get_onu_uint(phy_uint, &onu_uint);
    if(ret)
        return TRUE;
...
int
node_onu_db_onu_auth_info_tbl_get_free_onu_id(
                const uint8_t ni,
                uint16_t *p_onu_id)
{
...
int
node_onu_db_onu_auth_info_tbl_free_onu_id(
                const uint8_t ni,
...
int
node_onu_db_onu_auth_info_tbl_add(
                const uint8_t ni,
                const uint16_t onu_id,
                ...
                const uint32_t srvprofile_id)
{
    ...
    onu_uint = TERM_ID_2_TERM_UINT(ni, onu_id);
    if(!VALID_TERM_UINT(onu_uint))
        return SQLITE_PERM;
...
int
node_onu_db_onu_auth_info_tbl_auth_mode_get(
                const uint8_t ni,
                const uint16_t onu_id,
                uint8_t *p_auth_mode)
{
    ...
    snprintf(cmd_str, sizeof(cmd_str),
        "select * from onu_auth_info_tbl where onu_uint = %d", TERM_ID_2_TERM_UINT(ni, onu_id));
...
int
node_onu_db_onu_auth_info_tbl_get_first(
                uint32_t *p_onu_uint,
                uint8_t *p_auth_mode,
                uint8_t *p_deactive)
{
...
int
node_onu_db_onu_auth_info_tbl_set_online(
                const uint32_t onu_uint,
                const uint32_t phy_uint)
{
    int ret;
...
int
node_onu_db_onu_auth_info_tbl_set_offline(
                const uint32_t onu_uint,
                uint8_t *p_aging_flag)
{
    int ret;
    ...
    /* check aging */
    *p_aging_flag = 0;
    node_onu_db_onu_auth_sn_pwd_tbl_aging(onu_uint, p_aging_flag);
...
int
node_onu_db_onu_auth_info_tbl_set_offline_by_phy_uint(
                const uint32_t phy_uint,
                uint8_t *p_aging_flag)
{
    int ret;
    ...
    *p_aging_flag = 0;
    ...
    return node_onu_db_onu_auth_info_tbl_set_offline(onu_uint, p_aging_flag);
}


int
node_onu_db_onu_auth_info_tbl_set_dg_time(
                const uint32_t onu_uint)
{
    int ret;
    char cmd_str[MAX_SQLITE_CMD_STR_SIZE];
...
int
node_onu_db_onu_auth_info_tbl_set_desc(
                const uint32_t onu_uint,
...
int
node_onu_db_onu_auth_info_tbl_set_deactive_flag(
                const uint32_t onu_uint,
...
int
node_onu_db_onu_auth_info_tbl_get_phy_uint(
                const uint32_t onu_uint,
                uint32_t *p_phy_uint)
{
    int ret;
...
int
node_onu_db_onu_auth_info_tbl_get_onu_uint(
                const uint32_t phy_uint,
                uint32_t *p_onu_uint)
{
    int ret;
...
int
node_onu_db_onu_auth_info_tbl_get_deactive_flag(
                const uint32_t onu_uint,
                uint8_t *p_deactive,
                uint8_t *p_auth_mode)
{
...
int
node_onu_db_onu_auth_info_tbl_get_profile(
                const uint32_t onu_uint,
...
int
node_onu_db_onu_auth_info_tbl_get_line_profile_counter(
                const uint32_t lineprofile_id,
...
int
node_onu_db_onu_auth_info_tbl_get_srv_profile_counter(
                const uint32_t srvprofile_id,
                uint16_t *p_bind_counter)
{
    int ret;
...
int
node_onu_db_onu_auth_info_tbl_get_first_deactive(
...
int
node_onu_db_onu_auth_info_tbl_get_first_active(
                const uint32_t onu_uint_end,
                uint32_t *p_onu_uint,
                uint8_t *p_auth_mode)
{
...
int
node_onu_db_onu_auth_info_tbl_get_first_online(
                const uint32_t onu_uint_end,
                uint32_t *p_onu_uint,
                uint32_t *p_phy_uint)
{
...
int
node_onu_db_onu_auth_info_tbl_get_by_desc(
                const char *p_desc,
                uint32_t *p_onu_uint,
                uint8_t *p_auth_mode)
{
...
int
node_onu_db_onu_auth_info_tbl_get(
                const uint32_t onu_uint,
                uint32_t *p_phy_uint,
...
int
node_onu_db_onu_auth_info_tbl_del(
                const uint32_t onu_uint)
{
    int ret;
    char cmd_str[MAX_SQLITE_CMD_STR_SIZE];
...
static int
node_create_onu_auth_info_tbl(void)
{
    int ret;
    char * p_errmsg = NULL;

    ret = sqlite3_exec(p_node_onu_db, "drop table if exists onu_auth_info_tbl", 0, 0, 0);
    EXIT_ON_SQLITE(p_node_onu_db, ret, SQLITE_OK, "drop onu_auth_info_tbl failed", ret);
    ...
    ret = sqlite3_exec(p_node_onu_db, "create table "
                                            "onu_auth_info_tbl("
                                                ...
                                                "FOREIGN KEY(phy_uint) REFERENCES onu_phy_info_tbl(phy_uint))",
                                            NULL, NULL, &p_errmsg );
    EXIT_ON_SQLITE(p_node_onu_db, ret, SQLITE_OK, "create onu_auth_info_tbl failed", ret);
...
static int
node_onu_db_onu_auth_check_expire(
                const NODE_ONU_AUTH_TIME_MODE_E time_mode,
                ...
                const uint8_t authed_flag)
{
    ...
    switch(time_mode)
    {
        case NODE_ONU_AUTH_TIME_MODE_ALWAYS:
            return 0;

        case NODE_ONU_AUTH_TIME_MODE_ONCE_NO_AGING:
            if(!authed_flag)
                return 0;
            break;

        case NODE_ONU_AUTH_TIME_MODE_ONCE_AGING:
                ...
                uint8_t *p_aging_flag)
{
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        if(!node_onu_db_onu_auth_check_expire(time_mode, expire_at, authed_flag))
            break;
...
int
node_onu_db_onu_auth_sn_pwd_tbl_add(
                const uint8_t ni,
                const uint16_t onu_id,
                const NODE_ONU_AUTH_MODE_E auth_mode,
                const char *p_serial_number,
                ...
                const uint32_t srvprofile_id)
{
    ...
    char pwd_str[MAX_PWD_STR_SIZE];
    struct sysinfo sys_info;
    uint32_t onu_uint = TERM_ID_2_TERM_UINT(ni, onu_id);

    if(!VALID_TERM_UINT(onu_uint) || !p_serial_number)
        return SQLITE_PERM;
    ...
    if(time_mode == NODE_ONU_AUTH_TIME_MODE_ONCE_AGING && !sec_duration)
        return SQLITE_PERM;

    if(!node_does_sn_offline(ni, p_serial_number))
        return SQLITE_PERM;

    node_sn_to_str(p_serial_number, sn_str, MAX_SN_STR_SIZE);

    ret = node_onu_db_onu_auth_info_tbl_add(ni, onu_id, auth_mode, lineprofile_id, srvprofile_id);
    if(ret)
        return ret;
    ...
    do
    {
        ...
        if(time_mode != NODE_ONU_AUTH_TIME_MODE_ONCE_AGING)
...
int
node_onu_db_onu_auth_sn_pwd_tbl_check(
                const char *p_serial_number,
                const char *p_password,
                uint32_t *p_onu_uint,
                uint8_t *p_auth_mode)
{
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        *p_auth_mode = sqlite3_column_int(stmt, ONU_AUTH_SN_PWD_TBL_STMT_IDX_AUTH_MODE);
        ...
        {
            if(node_pwd_to_str(p_password, pwd_str, sizeof(pwd_str)) ||
    ...
    if(aging_flag)
    {
        ...
        ret = sqlite3_exec(p_node_onu_db, cmd_str, 0, 0, 0);
        EXIT_ON_SQLITE(p_node_onu_db, ret, SQLITE_OK, cmd_str, rc); //return rc, not ret ?
...
int
node_onu_db_onu_auth_sn_pwd_tbl_set_authed(
                const uint32_t onu_uint)
{
    int ret;
    char cmd_str[MAX_SQLITE_CMD_STR_SIZE];
...
int
node_onu_db_onu_auth_sn_pwd_tbl_get_sn_pwd(
                const uint32_t onu_uint,
                uint8_t *p_auth_mode,
                char *p_serial_number,
                char *p_password)
{
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW)
    {
        ...
        *p_auth_mode = sqlite3_column_int(stmt, ONU_AUTH_SN_PWD_TBL_STMT_IDX_AUTH_MODE);
        node_str_to_sn(sqlite3_column_text(stmt, ONU_AUTH_SN_PWD_TBL_STMT_IDX_SN), p_serial_number);
        node_str_to_pwd(sqlite3_column_text(stmt, ONU_AUTH_SN_PWD_TBL_STMT_IDX_PWD), p_password);
        sqlite3_finalize(stmt);
        NODE_DB_UNLOCK;
...
int
node_onu_db_onu_auth_sn_pwd_tbl_get_onu_uint_by_sn(
                const char *p_serial_number,
                uint32_t *p_onu_uint,
                uint8_t *p_auth_mode)
{
...
int
node_onu_db_onu_auth_sn_pwd_tbl_get_onu_uint_by_pwd_first(
...
static int
node_create_onu_auth_sn_pwd_tbl(void)
{
    int ret;
    char * p_errmsg = NULL;

    ret = sqlite3_exec(p_node_onu_db, "drop table if exists onu_auth_sn_pwd_tbl", 0, 0, 0);
    EXIT_ON_SQLITE(p_node_onu_db, ret, SQLITE_OK, "drop onu_auth_sn_pwd_tbl failed", ret);
    ...
    ret = sqlite3_exec(p_node_onu_db, "create table "
                                            ...
                                            NULL, NULL, &p_errmsg );
    EXIT_ON_SQLITE(p_node_onu_db, ret, SQLITE_OK, "create onu_auth_sn_pwd_tbl failed", ret);
...
int
node_onu_db_onu_fliter_tbl_set_switch(
                uint8_t sw_val)
{
...
int
node_onu_db_onu_fliter_tbl_get_switch(
                uint8_t *p_sw)
{
    if (!p_sw)
        return SQLITE_PERM;
...
int
node_onu_db_onu_fliter_tbl_set_mode(
                uint8_t mode)
{
...
int
node_onu_db_onu_fliter_tbl_get_mode(
                uint8_t *p_mode)
{
    if (!p_mode)
        return SQLITE_PERM;
...
int
node_onu_db_onu_fliter_tbl_get_records_count(
                uint32_t *p_count)
{
    if (!p_count)
        return SQLITE_PERM;
...
static int
node_onu_db_onu_filter_tbl_conflict_check(
...
int
node_onu_db_onu_fliter_tbl_add(
                const uint8_t mask_len,
                const char *p_sn_str)
{
    int ret;
    ...
    if(!node_onu_db_onu_filter_tbl_conflict_check(mask_len, p_sn_str))
        return SQLITE_PERM;
...
int
node_onu_db_onu_filter_tbl_del(
...
int
node_onu_db_onu_filter_tbl_del_all(void)
{
     int ret;
    char cmd_str[MAX_SQLITE_CMD_STR_SIZE];
...
//search the sn in db
int
node_onu_db_onu_filter_tbl_sn_hit_check(
                const char *p_serial_number)
{
    int ret;
    char cmd_str[MAX_SQLITE_CMD_STR_SIZE];
...
int
node_onu_db_onu_filter_tbl_get_first(
                const int start_idx,
                int *p_idx,
                char *p_sn_str,
...
static int
node_create_onu_filter_tbl(void)
{
    int ret;
    char * p_errmsg = NULL;

    ret = sqlite3_exec(p_node_onu_db, "drop table if exists onu_filter_tbl", 0, 0, 0);
    EXIT_ON_SQLITE(p_node_onu_db, ret, SQLITE_OK, "drop onu_filter_tbl failed", ret);
    ...
    EXIT_ON_SQLITE(p_node_onu_db, ret, SQLITE_OK, "create onu_filter_tbl failed", ret);
...
node_db_init(void)
{
    ...
    if (sem_init(&node_onu_db_sem, 0, 1) != 0 )
    {
    ...
    //ret = sqlite3_open(":tfdrv_db:", &p_node_onu_db);
    ret = sqlite3_open_v2("tfdrv_db", &p_node_onu_db, SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE | SQLITE_OPEN_FULLMUTEX, NULL);
    EXIT_ON_SQLITE(p_node_onu_db, ret, SQLITE_OK, "Open db failed", ret);

    //sqlite3_config(SQLITE_CONFIG_MULTITHREAD);

    //ret = sqlite3_exec(p_node_onu_db, "PRAGMA synchronous = OFF; ", 0, 0, 0);
    //EXIT_ON_SQLITE(p_node_onu_db, ret, SQLITE_OK, "Init db failed", ret);

    ret = sqlite3_exec(p_node_onu_db, "PRAGMA foreign_keys = ON; ", 0, 0, 0);
    EXIT_ON_SQLITE(p_node_onu_db, ret, SQLITE_OK, "Init db failed", ret);

    autofind_aging_duration = DEFAULT_AUTOFIND_AGING_DURATION;

    node_create_onu_black_tbl();
    node_create_onu_phy_info_tbl();

    node_create_onu_auth_sn_pwd_tbl();
    node_create_onu_auth_info_tbl();

    node_create_onu_filter_tbl();

    return 0;
}

int
node_onu_db_uninit(void)
{
    return 0;
}
