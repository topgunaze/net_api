                ...
                const uint32_t          arg1)
{
    ...
    p_new_worker = (NODE_WORKER *)malloc(sizeof (NODE_WORKER));
    ...
    net_bitlist_set(p_thread_pool->worker_bitmap[PHY_UINT_2_PHY_ID(key)], PHY_UINT_2_PHY_ID(key));
...
//节点设备是否正在执行任务
static int
thread_pool_node_worker_is_busy(THREAD_POOL *p_thread_pool, uint32_t key)
{
    uint16_t idx;
            ...
            for(bit_idx = 7; bit_idx >= 0; bit_idx--)
            {
                if((p_thread_pool->worker_bitmap[port_idx][byte_idx] >> bit_idx) & 0x01U)
                {
                    *p_key = PHY_ID_2_PHY_UINT(port_idx, byte_idx*8+7-bit_idx);
                    ...
                    //节点的任务是否正在执行
                    if (thread_pool_node_worker_is_busy(p_thread_pool, *p_key))
                    {
                         continue;//遍历下一个节点
                    }
                    ...
                    //该类型任务是否超过最大线程数
                    if (thread_pool_node_worker_thread_is_busy(p_thread_pool, p_worker))
                    {
                        continue;//遍历下一个节点
                    }
                    ...
                    {
                        //刷新数图
                        net_bitlist_clear(p_thread_pool->worker_bitmap[port_idx], byte_idx*8+7-bit_idx);
                    }
...
//每次遍历onu依赖于前次
static NODE_WORKER*
thread_pool_node_worker_polling_get(
                                    THREAD_POOL  *p_thread_pool, 
                                    uint32_t     *p_key)
{
                ...
                void *arg)
{
    ...
    while (1)
    {
        ...
        p_worker = thread_pool_node_worker_get(p_thread_pool, &key);
...
static void
thread_pool_node_worker_destory(
                THREAD_POOL *p_thread_pool)
{
    int32_t         port_idx, byte_idx, bit_idx;
    uint32_t        key;
    NODE_WORKER *p_worker, *p_tmp_worker;
...
int
thread_pool_destory(THREAD_POOL *p_thread_pool)
{
    uint32_t        idx;
    ...
    /*销毁等待队列*/
    thread_pool_node_worker_destory(p_thread_pool);
...
main(int argc, const char*argv[])
{
    ...
    while(1)
    {
        ...
        {
            case '1':
                thread_pool_add_worker(&pt_pool, test_proc_handle, 0, 1, seq++);
                break;
            case '2':
                thread_pool_add_worker(&pt_pool, test_proc_handle, 0, 2, seq++);
                break;
            case '3':
                thread_pool_add_worker(&pt_pool, test_proc_handle, 0, 3, seq++);
                break;
            case '4':
                thread_pool_add_worker(&pt_pool, test_proc_handle, 0, 4, seq++);
                break;

            case '5':
                thread_pool_add_worker(&pt_pool, test_proc_handle, 1, 1, seq++);
                break;
            case '6':
                thread_pool_add_worker(&pt_pool, test_proc_handle, 1, 2, seq++);
                break;
            case '7':
                thread_pool_add_worker(&pt_pool, test_proc_handle, 1, 3, seq++);
                break;
            case '8':
                thread_pool_add_worker(&pt_pool, test_proc_handle, 1, 4, seq++);
                break;

            case '9':
                thread_pool_clear_worker(&pt_pool, 1, WORKER_TYPE_COMMON);
                thread_pool_clear_worker(&pt_pool, 1, WORKER_TYPE_UPGRADE);
                break;

            case 'd':
                thread_pool_destory(&pt_pool);
                break;
