...
/***Begin: Add by steven.tian on 2015/12/30*************/
/* Clear and free all allocated data. */
size_t
buffer_data_size(struct buffer *b)
{
    struct buffer_data *data;
    size_t size = 0;

    for (data = b->head; data; data = data->next) {
        if (size + data->cp - data->sp < size)
            break;
...
/***End: Add by steven.tian on 2015/12/30*************/
		     ...
		     int erase_flag, int no_more_flag)
{
  ...
  char more[] = "  --More ( Press 'Q' to quit )-- ";
  ...
  for (data = b->head; data && (height > 0); data = data->next)
    {
        //printf("%s %d, height=%d, cp=%d, sp=%d\r\n", __func__, __LINE__, height, data->cp, data->sp);
      size_t cp;
	  ...
	  if (data->data[cp] == '\r')
	    ...
	    column++;
	  cp++;
	  flush_size++;/* record flush data size */
        }
     ...
     example: Solaris2.6 are defined IOV_MAX size at 16.     */
  {
   ...
   if (nbytes < 0 && ERRNO_IO_RETRY (save_errno))
   {
      int bufs = 0;
      data = b->head;
      while (data && (data->sp == data->cp))
      {
         if (data != b->head)
         {
            bufs += data->sp;
      ...
      if (bufs < flush_size)
      {
         if (b->head->sp >= (flush_size - bufs))
         {
             b->head->sp = b->head->sp - (flush_size - bufs);
             data = b->head->next;
             while (data && (data->sp == data->cp))
              {
                 data->sp = 0;
...
#define MAX_FLUSH 131072
  ...
  if ((ssize_t)(written = writev(fd,iov,iovcnt)) < 0)
	...
	/* Calling code should try again later. */
      ...
      zlog_warn("PID[%d] %s: write error on fd %d: %s",
		getpid(), __func__, fd, safe_strerror(errno));
      return BUFFER_ERROR;
    }
