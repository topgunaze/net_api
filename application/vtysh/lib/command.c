...
cmd_make_strvec (const char *string)
{
    ...
    while (1) {
        ...
        while (!(isspace ((int) *cp) || *cp == '\r' || *cp == '\n') && *cp != '\0'){
            //stephen.liu add , for \"STRING\", 20161011
            if(*cp == '\"'){
              cp++;
              while(*cp != '\"' && *cp != '\0')
                cp++;
...
format_parser_begin_multiple (struct format_parser_state *state)
{
    ...
    key_multiple = vector_init (VECTOR_MIN_SIZE);
    vector_set (token->multiple, key_multiple);
    
    vector_set (state->curvect, token);
    if (state->curvect != state->topvect)
        state->intvect = state->curvect;
    state->curvect = key_multiple;
    //printf("%s %d\n", __func__, __LINE__);
}

/*stephen, for save old parse func
    20151118 */
void
format_parser_begin_multiple_old (struct format_parser_state *state)
{
    struct cmd_token *token;
...
format_parser_handle_pipe (struct format_parser_state *state)
{
    ...
    if (state->in_multiple) {
        state->just_read_word = 0;
        state->cp++;
        //printf("%s %d key:%d\n", __func__, __LINE__, state->in_keyword);
        if(state->in_keyword == 2){
            //stephen.liu add, 20151118
            keyword_token = vector_slot (state->intvect,
                                         vector_active (state->intvect) - 1);
            keyword_vect = vector_init (VECTOR_MIN_SIZE);
            vector_set (keyword_token->multiple, keyword_vect);
            state->curvect = keyword_vect;
        }else{
            //stephen.liu add, 20151118
...
/* stephen.liu backup, 20151118*/
void
format_parser_handle_pipe_old (struct format_parser_state *state)
{
    struct cmd_token *keyword_token;
    vector keyword_vect;
...
install_element (enum node_type ntype, struct cmd_element *cmd)
{
    ...
    //i++;
    //printf("num=%d\n", i);
	cmd->right = 0; /* add by keith.gong 20151102 */
    cmd->style = 0;
}
/* Start add by keith.gong 20151102 */
void
install_element_with_right(enum node_type ntype, int right, struct cmd_element *cmd)
{
    struct cmd_node *cnode;
...
/* comamnd version control flag
    eg. install_element_v(node, version , cmd)
   also, if service port version, you must open macro (SERVICE_PORT)
*/
static int command_version = 0;

void setCommandVersion(int *src_v, int v)
{
    /* if you want more version control, open this */
    /* stephen.liu 20160719 */
    //*src_v |= (0x1 << v);
    /* now, we just support one version */
    *src_v = v;
}

int checkCommandVersion(int v)
{
    if(command_version == v){
        return TRUE;
    }
    return FALSE;
}

/* End add by keith.gong 20151102 */
/* Start add by gerhard.lao 20160707 */
void
install_element_with_style(enum node_type ntype, int style, struct cmd_element *cmd)
{
    struct cmd_node *cnode;
...
/* End add by gerhard.lao 20160707 */
...
config_write g_config_write_host ;

void vty_set_config_write(config_write s)
{
    g_config_write_host = s;
}
...
config_write_host (struct vty *vty)
{
    ...
    if (zlog_default && zlog_default->default_lvl != LOG_DEBUG) {
    ...
    if (host.logfile && zlog_default && (zlog_default->maxlvl[ZLOG_DEST_FILE] != ZLOG_DISABLED)) {
    ...
    if (zlog_default && zlog_default->maxlvl[ZLOG_DEST_STDOUT] != ZLOG_DISABLED) {
    ...
    if (zlog_default && zlog_default->maxlvl[ZLOG_DEST_MONITOR] == ZLOG_DISABLED)
        vty_out (vty, "no log monitor%s", VTY_NEWLINE);
    else if (zlog_default && zlog_default->maxlvl[ZLOG_DEST_MONITOR] != zlog_default->default_lvl)
        vty_out (vty, "log monitor %s%s", zlog_priority[zlog_default->maxlvl[ZLOG_DEST_MONITOR]], VTY_NEWLINE);

    if (zlog_default && zlog_default->maxlvl[ZLOG_DEST_SYSLOG] != ZLOG_DISABLED) {
    ...
    if (zlog_default && zlog_default->facility != LOG_DAEMON)
        vty_out (vty, "log facility %s%s", facility_name (zlog_default->facility), VTY_NEWLINE);

    if (zlog_default && zlog_default->record_priority == 1)
        vty_out (vty, "log record-priority%s", VTY_NEWLINE);

    if (zlog_default && zlog_default->timestamp_precision > 0)
    ...
    #endif
    if(g_config_write_host)
        g_config_write_host(vty);
...
cmd_mac_addr_match (const char *str)
{
    ...
    for (;;) {
        ...
        //printf("%s %d, buf=%s, d=%d\r\n", __func__, __LINE__, buf, atoi(buf)); //stephen.liu
        if (atoi (buf) > 255)
            return no_match;
...
cmd_hex_range_match (const char *range, const char *str)
{
    ...
    char buf[DECIMAL_STRLEN_MAX + 1];
    char *endptr = NULL;
    unsigned long min=0, max=0, val=0;
    int base = 0;

    if (str == NULL)
        return 1;
    //stephen.liu 20160815
    if((str[0] == '0')&&(str[1] == 'x')){
        base = 16;
    }else{
        base = 10;
    }
    //printf("range:%s, str:%s\n", range, str);
    ...
    strncpy (buf, range, p - range);
    buf[p - range] = '\0';
    max = strtoul (buf, &endptr, base);
    if (*endptr != '\0')
        return 0;
    //stephen.liu
    //printf("6val:%d, min:%d, max:%d\n", val, min, max);
    if (val < min || val > max)
        return 0;
...
static int
cmd_string_match (const char *string, const char *str)
{
  char *p = str;
...
static int
cmd_time_match (const char *string, const char *str)
{
    char *p = str;
    ...
    if (str == NULL)
        return 1;
    if(!strcmp(string, "YYYY/MM/DD-HH:MM:SS"))      {
        //printf("%s %d\r\n", __func__, __LINE__);
        while (*p != '\0') {
            if (*p == '/') {
                if (s >= 2)
        ...
        if ((n = sscanf(str, "%d/%d/%d-%d:%d:%d", &year, &month, &day, &hour, &min, &sec) )!= 6){
            //printf("%s %d, n = %d, year=%d, month=%d\n", __func__, __LINE__, n, year, month);
            if(n){
                //printf("%s %d\r\n", __func__, __LINE__);
                if(n >= 1){
                    if(!CMD_TIME_YEAR_CHECK(year)){
                        return 0;
                    }
                }
                if(n >= 2){
                    if(!CMD_TIME_MON_CHECK(month)){
                        return 0;
                    }
                    if(month==2){
        ...
        //printf("%s %d\r\n", __func__, __LINE__);
        #if 0
        sprintf(tstr, "%d/%d/%d-%d:%d:%d", year, month, day, hour, min, sec);
       // printf("%s %d ,tstr=%s, str=%s, strlen(tstr)=%d, strlen(str)=%d\r\n", __func__, __LINE__, tstr, str, strlen(tstr), strlen(str));
        if(strlen(tstr) != strlen(str))
            return 0;
        #endif
        //printf("%s %d\r\n", __func__, __LINE__);
        if(!CMD_TIME_YEAR_CHECK(year)\
            &&!CMD_TIME_MON_CHECK(month)){
            return 0;
        }
        if(month==2){
            if( year % 4 == 0 && ( (year % 100) != 0 || (year % 400) == 0)){
    ...
    }else if(!strcmp(string, "YYYY/MM/DD")){
        //printf("%s %d\r\n", __func__, __LINE__);
        while (*p != '\0') {
            if (*p == '/') {
                if (s >= 2)
        ...
        if ((n = sscanf(str, "%d/%d/%d", &year, &month, &day) )!= 3){
            //printf("%s %d, n = %d, year=%d, month=%d\n", __func__, __LINE__, n, year, month);
            if(n){
                //printf("%s %d\r\n", __func__, __LINE__);
                if(n >= 1){
        ...
        #if 0
        //printf("%s %d\r\n", __func__, __LINE__);
        sprintf(tstr, "%d/%d/%d", year, month, day);
        //printf("%s %d ,tstr=%s, str=%s, strlen(tstr)=%d, strlen(str)=%d\r\n", __func__, __LINE__, tstr, str, strlen(tstr), strlen(str));
        if(strlen(tstr) != strlen(str))
            return 0;
        #endif
    ...
    }else if(!strcmp(string, "HH:MM:SS")){
        //printf("%s %d\r\n", __func__, __LINE__);
        while (*p != '\0') {
        ...
        if ((n = sscanf(str, "%d:%d:%d", &hour, &min, &sec) )!= 3){
           // printf("%s %d, n = %d ,hour=%d, min=%d\n", __func__, __LINE__, n, hour, min);
            if(n){
                //printf("%s %d\r\n", __func__, __LINE__);
                if(n >= 1)
                if((hour > 23) || hour == 0)
        ...
        //printf("%s %d\r\n", __func__, __LINE__);
        #if 0
        sprintf(tstr, "%d:%d:%d",  hour, min, sec);
        if(strlen(tstr) != strlen(str))
            return no_match;
        #endif
        if(hour > 23 || hour < 0){
...
static enum match_type
cmd_word_match (struct cmd_token *token, enum filter_type filter, const char *word)
{
    const char *str;
    enum match_type match_type;
    char *s = NULL;
    char arr[SINGLE_WORD_SIZE] ;
    int i = 0;

    //stephen.liu 20151130
    if (!token||!token->cmd)
        return no_match;
    ...
    if (CMD_VARARG (str)) {
        ...
        if (cmd_hex_range_match (str, word))
            return range_match;
    }
    else if (CMD_STRING(str)){
        if (cmd_string_match(str, word))
            return string_match;
    }
    ...
    //stephen.liu add
        ...
        match_type = cmd_mac_addr_match (word);
        if ((filter == FILTER_RELAXED && match_type != no_match)
            || (filter == FILTER_STRICT && match_type == exact_match))
            return mac_match;
    }
    //stephen.liu add
    else if (CMD_TIME (str)){
        match_type = cmd_time_match (str, word);
        if ((filter == FILTER_RELAXED && match_type != no_match)
            || (filter == FILTER_STRICT && match_type == exact_match))
            return time_match;
    }
        ...
        for (s = (char *)word, i = 0; (*s != '\0')&&(i<SINGLE_WORD_SIZE); i++, s++) {
          arr[i] = tolower(*s);
        }
        arr[i] = '\0';

        if (filter == FILTER_RELAXED && !strncmp (str, arr, strlen (arr))) {
            if (!strcmp (str, arr))
                return exact_match;
            return partly_match;
        }
        if (filter == FILTER_STRICT && !strcmp (str, arr))
            return exact_match;
    }
...
static int
push_argument(int *argc, const char **argv, char **arg)
{
    char *cp = NULL, *s = NULL;
...
static void
cmd_matcher_record_match (struct cmd_matcher *matcher, enum match_type match_type, struct cmd_token *token)
{
...
cmd_matcher_match_terminal (struct cmd_matcher *matcher, struct cmd_token *token, int *argc, const char **argv)
{
    const char *word;
...
enum matcher_rv
cmd_matcher_match_multiple_old (struct cmd_matcher *matcher,
                            struct cmd_token *token,
                            ...
                            int *argc, const char **argv)
{
    ...
    struct cmd_token *word_token;
    enum match_type word_match;
    vector word_key, key_vector;
    unsigned int keyword_found = 0;
    int rv;
    ...
    if (!cmd_matcher_words_left (matcher))
        return MATCHER_INCOMPLETE;
    int i = 0;
    //printf("matcher->word_index=%d\n", matcher->word_index);
    //for(i = 0; i <= matcher->word_index; i++)
    //printf("%s ", vector_slot(matcher->vline, i));
    //printf("\n");
    word = cmd_matcher_get_word (matcher);
    for (multiple_index = 0;
         multiple_index < vector_active (token->multiple); multiple_index++) {
        word_key = vector_slot (token->multiple, multiple_index);
        word_token = vector_slot (word_key, 0);
        //printf("word:%s, cmd:%s\n", word, word_token->cmd);
        word_match = cmd_word_match (word_token, matcher->filter, word);
        if (word_match == no_match)
            continue;
    ...
    key_vector = vector_slot (token->multiple, keyword_found);
    /* the keyword itself is at 0. We are only interested in the arguments,
     * so start counting at 1. */
    for (i = 1; i < vector_active (key_vector); i++) {
        word_token = vector_slot (key_vector, i);
    
        switch (word_token->type) {
        case TOKEN_TERMINAL:
            rv = cmd_matcher_match_terminal (matcher, word_token,
                                             argc, argv);
            //printf("l term --- rv=%d,index=%d\n", rv, matcher->word_index);
            break;
        case TOKEN_MULTIPLE:
...
cmd_matcher_read_keywords (struct cmd_matcher *matcher,
                           struct cmd_token *token, vector args_vector)
{
    unsigned int i,j,m;
    unsigned long keyword_mask,check_mask;
    unsigned int keyword_found=0,key_j=0xff,key_wf=0xff;
    enum match_type keyword_match;
    enum match_type word_match;
    vector keyword_vector,key_multi, key_vector;
    struct cmd_token *word_token, *wtoken;
    //const char *arg;
    const char *word;
    int keyword_argc;
    ...
    while (1) {
        ...
        for (i = 0; i < vector_active (token->keyword); i++) {
            ...
            if(word_token && word_token->type == TOKEN_MULTIPLE){
                //printf("%s %d\n", __func__, __LINE__);
                key_multi = word_token->multiple;
                for(j = 0; j < vector_active (word_token->multiple); j++){
                    keyword_vector = vector_slot (word_token->multiple, j);
                    
                    wtoken = vector_slot (keyword_vector, 0);
                    word_match = cmd_word_match (wtoken, matcher->filter, word);
                    if (word_match == no_match){
                        //printf("ccword no match:%s, word:%s\n",wtoken->cmd, word);
                        continue;
                    }
                    cmd_matcher_record_match (matcher, word_match, wtoken);
        ...
        //1 stephen.liu add 20151021, for keywords ,the last token while not be recognition
        for (m = 0; m < i; m++) {
            check_mask |= (1 << m);
        }
        if (keyword_mask == check_mask) {
            return MATCHER_OK;
        }
        check_mask = 0;

        if (keyword_found == (unsigned int) -1) {
            //printf("matcher read keywords none.\n");//stephen.liu
            return MATCHER_NO_MATCH;
        }
        matcher->word_index++;
        ...
        /* the keyword itself is at 0. We are only interested in the arguments,
         * so start counting at 1. */
		if(0xff == key_j || key_wf != 0xff){
        for (i = 1; i < vector_active (keyword_vector); i++) {
            word_token = vector_slot (keyword_vector, i);

            switch (word_token->type) {
            case TOKEN_TERMINAL:
                rv = cmd_matcher_match_terminal (matcher, word_token, &keyword_argc, keyword_argv);
                break;
            case TOKEN_MULTIPLE:
                rv = cmd_matcher_match_multiple (matcher, word_token, &keyword_argc, keyword_argv);
                break;
            case TOKEN_KEYWORD:
                assert (!"Keywords should never be nested.");
                break;
            ...
            word_token = vector_slot (keyword_vector, 0);
            if(word_token->type == TOKEN_MULTIPLE){
                key_vector = vector_slot (key_multi, key_j);
                /* the keyword itself is at 0. We are only interested in the arguments,
                        * so start counting at 1. */
                push_argument (&keyword_argc, keyword_argv, &word);
                for (i = 1; i < vector_active (key_vector); i++) {
                    wtoken = vector_slot (key_vector, i);

                    
                    switch (wtoken->type) {
                    case TOKEN_TERMINAL:
                        rv = cmd_matcher_match_terminal (matcher, wtoken,
                                                         &keyword_argc, keyword_argv);
                        break;
                    case TOKEN_MULTIPLE:
                        //rv = cmd_matcher_match_multiple (matcher, word_token,
...
cmd_matcher_build_keyword_args (struct cmd_matcher *matcher,
                                struct cmd_token *token, int *argc, const char **argv, vector keyword_args_vector)
{
    unsigned int i, j;
    ...
    for (i = 0; i < vector_active (token->keyword); i++) {
        ...
        if (vector_active (keyword_vector) == 1 && wtoken->type != TOKEN_MULTIPLE) {
            ...
            if (keyword_args) {
                ...
                for (j = 1; j < vector_active (keyword_vector); j++) {
                    ...
                    if ((word_token->type == TOKEN_TERMINAL && (CMD_VARARG (word_token->cmd)
...
static enum matcher_rv
cmd_matcher_match_keyword (struct cmd_matcher *matcher, struct cmd_token *token, int *argc, const char **argv)
{
    ...
    /* add by keith.gong 20151102 . input at least one element in the "{}"*/
    if (!cmd_matcher_words_left (matcher))
        return MATCHER_INCOMPLETE;
...
cmd_matcher_init (struct cmd_matcher *matcher,
                  ...
                  vector vline, unsigned int index, enum match_type *match_type, vector * match)
{
...
/* Start, add by keith.gong 20151102. for right control */
extern unsigned char 
cmd_element_access_allowed(int nResourceAccess, int nUserAccess)
{
...
/* End, add by keith.gong 20151102. for right control */
...
cmd_element_match (struct cmd_element *cmd_element,
                   ...
                   int style,
                   vector vline,
                   unsigned int index, enum match_type *match_type, vector * match, int *argc, const char **argv)
{
    struct cmd_matcher matcher;
    unsigned int token_index;
    enum matcher_rv rv = MATCHER_NO_MATCH;

    /* add by keith.gong 20151102. for right control */
    if(cmd_element->right && sign_user && sign_user->name[0] && cmd_element_access_allowed(cmd_element->right, sign_user->level) == FALSE)
    {   
        //printf("%s:%d name:%p,%s\r\n", __FUNCTION__, __LINE__,sign_user, sign_user ?  sign_user->name : "???");
        //printf("%s:%d string:%s right:%d user:%d\r\n", __FUNCTION__, __LINE__, cmd_element->string, cmd_element->right, sign_user->level);
        return MATCHER_NO_MATCH; 
    }

    /* add by gerhard.lao 20160707. for style control */
    if((style && cmd_element->style) && (cmd_element->style != style))
        return MATCHER_NO_MATCH; 
    ...
    if (matcher.word_index == vector_active (vline) - 1 && (!vector_slot (vline, matcher.word_index)
                                                            || !strlen ((char *)
...
cmd_vector_filter (vector commands,
                   ...
                   struct vty_user *sign_user,
                   int style,
                   vector vline, unsigned int index, enum match_type *match_type, vector * matches)
{
    unsigned int i;
    ...
    *matches = vector_init (VECTOR_MIN_SIZE);
    ...
    for (i = 0; i < vector_active (commands); i++)
        if ((cmd_element = vector_slot (commands, i)) != NULL) {
            ...
            matcher_rv = cmd_element_match (cmd_element, filter, sign_user, style,
                                            vline, index,
                                            &element_match, (vector *) & vector_slot (*matches, i), NULL, NULL);
 ...
 */
static int
cmd_is_complete (struct vty_user *sign_user, int style, struct cmd_element *cmd_element, vector vline)
{
    enum matcher_rv rv;

    rv = cmd_element_match (cmd_element, FILTER_RELAXED, sign_user, style, vline, -1, NULL, NULL, NULL, NULL);
 ...
 */
static int
cmd_parse (struct vty_user *sign_user, int style, struct cmd_element *cmd_element, vector vline, int *argc, const char **argv)
{
...
static int
is_cmd_ambiguous (vector cmd_vector, const char *command, vector matches, enum match_type type)
{
    unsigned int i;
    unsigned int j;
    const char *str = NULL;
            ...
            for (j = 0; j < vector_active (match_vector); j++)
                if ((cmd_token = vector_slot (match_vector, j)) != NULL) {
                    ...
                    case range_match:
                        ...
                        break;
                    case string_match:
                        if (cmd_string_match (str, command)){
                            if (matched && strcmp (matched, str) != 0)
                                return 1;
                        else
                            matched = str;
...
cmd_entry_function (const char *src, const char *dst)
{
    ...
    if (CMD_OPTION (dst) || CMD_VARIABLE (dst) || CMD_VARARG (dst) ||
        CMD_IPV4 (dst) || CMD_IPV4_PREFIX (dst) || CMD_HEX_RANGE (dst)||
        CMD_MAC (dst)||(CMD_STRING(dst))||(CMD_TIME(dst)))
        return NULL;
    ...
    if (src == NULL)
        return dst;
    memset(arr, 0x00, sizeof(arr));
    for (s = (char *)src, i = 0; (*s != '\0')&&(i<SINGLE_WORD_SIZE); i++, s++) {
      arr[i] = tolower(*s);
    }
    arr[i] = '\0';

    /* Matched with input string. */
    if (strncmp (arr, dst, strlen (arr)) == 0)
        return dst;
...
cmd_entry_function_desc (const char *src, const char *dst)
{
    ...
    if (src == NULL)
        return dst;
    //stephen.liu for AAA input, 20161019
    memset(arr, 0x00, sizeof(arr));
    for(s = (char *)src,i = 0; (*s != '\0')&&(i<SINGLE_WORD_SIZE); i++, s++){
      arr[i] = tolower(*s);
    }
...
cmd_try_do_shortcut (enum node_type node, char *first_word)
{
    if (first_word != NULL &&
        node != AUTH_NODE &&
        node != VIEW_NODE &&
        node != AUTH_ENABLE_NODE && node != ENABLE_NODE && node != RESTRICTED_NODE && 0 == strcmp ("do", first_word))
        return 1;
    return 0;
}
...
#define INIT_MATCHVEC_SIZE 10
    ...
    for (i = 0; i <= index; i++) {
        ...
        ret = cmd_vector_filter (cmd_vector, FILTER_RELAXED, &vty->sign_user, vty->sign_user.style, vline, i, &match, &matches);
    ...
    for (i = 0; i < vector_active (matches); i++)
        if ((cmd_element = vector_slot (cmd_vector, i)) != NULL) {
            ...
            if (last_word == NULL || !strlen (last_word)) {
                ...
                if (cmd_is_complete (&vty->sign_user, vty->sign_user.style, cmd_element, vline_trimmed)
        ...
        if ((match_vector = vector_slot (matches, i))) {
            ...
            for (j = 0; j < vector_active (match_vector); j++)
                if ((token = vector_slot (match_vector, j))) {
                    if ((string = cmd_entry_function (vector_slot (vline, index), token->cmd)))
                    {
...
#ifdef _ENABLE_VTY_SPACE_EXPAND_    /* Start. Add by keith.gong. 20151103 */
                    else if(!match_flag)
                    {
                        enum match_type match = cmd_word_match(token, FILTER_STRICT, vector_slot (vline, index));
                        //printf("%s:%d ---------++++++--- cmd:%s str:%s match:%d\r\n", __FUNCTION__, __LINE__, 
                        //    token->cmd,(char*)vector_slot (vline, index),  match);
...
#ifdef _ENABLE_VTY_SPACE_EXPAND_    /* Start. Add by keith.gong. 20151022 */
    if (vector_slot (vline, index) != NULL) {
        for (i = 0; match_str[i] != NULL; i++) {
            if (!strcmp (vector_slot (vline, index), match_str[i])) {
                *status = CMD_COMPLETE_FULL_MATCH;

                if (match_str)
                    vector_only_index_free (match_str);
                return NULL;
            }
        }
    }
...
extern execute_func_to_all_daemon g_func_to_all_daemon;
excute_func g_exec_daemon;
/* Execute command by argument vline vector. */
static int
cmd_execute_command_real (vector vline, enum filter_type filter, struct vty *vty, struct cmd_element **cmd)
{
    unsigned int i;
    unsigned int index;
    vector cmd_vector;
    ...
    for (index = 0; index < vector_active (vline); index++) {
        ...
        ret = cmd_vector_filter (cmd_vector, filter, &vty->sign_user, vty->sign_user.style, vline, index, &match, &matches);
    ...
    for (i = 0; i < vector_active (cmd_vector); i++)
        if ((cmd_element = vector_slot (cmd_vector, i))) {
            if (cmd_is_complete (&vty->sign_user, vty->sign_user.style, cmd_element, vline)) {
    ...
    ret = cmd_parse (&vty->sign_user, vty->sign_user.style, matched_element, vline, &argc, argv);
    if (ret != CMD_SUCCESS)
        return ret;
    ...
    if (matched_element->daemon) {
        //#ifdef VTYSH_ENABLE
        //printf("has excute local\n");
        //ret = CMD_SUCCESS;
        //if(matched_element->func)
        //    ret = (*matched_element->func) (matched_element, vty, argc, argv);
        //printf("%s:%d excute remote:%s\r\n", __FUNCTION__, __LINE__, matched_element->string);
        if (g_exec_daemon)
            return g_exec_daemon (vty, matched_element, argc, argv);
        //#endif
        //else return ret;
        return CMD_SUCCESS_DAEMON;
    }
    //printf("Execute matched command.%s\r\n", matched_element->string);
...
cmd_execute_command (vector vline, struct vty *vty, struct cmd_element **cmd, int vtysh)
{
    ...
    saved_ret = ret = cmd_execute_command_real (vline, FILTER_STRICT, vty, cmd);
 ...
 */
int
cmd_execute_command_strict (vector vline, struct vty *vty, struct cmd_element **cmd)
{
...
config_from_file (struct vty *vty, FILE * fp, unsigned int *line_num)
{
    ...
    *line_num = 0;
    ...
    while (fgets (vty->buf, VTY_BUFSIZ, fp)) {
        ...
        while (ret != CMD_SUCCESS && ret != CMD_WARNING && ret != CMD_ERR_NOTHING_TODO && vty->node != CONFIG_NODE) {
...
2. Partial help is provided when an abbreviated argument is entered%s\
   ...
   (e.g. 'show me?'.)%s%s", VTY_NEWLINE, VTY_NEWLINE, VTY_NEWLINE, VTY_NEWLINE, VTY_NEWLINE, VTY_NEWLINE, VTY_NEWLINE, VTY_NEWLINE, VTY_NEWLINE, VTY_NEWLINE, VTY_NEWLINE, VTY_NEWLINE, VTY_NEWLINE);
    return CMD_SUCCESS;
}
       ...
       "Write to configuration file\n")
{
    ...
    if (host.config == NULL) {
        vty_out (vty, "PID[%d] Can't save to configuration file, using vtysh.%s", getpid(), VTY_NEWLINE);
        return CMD_WARNING;
    }
    ...
    if (fd < 0) {
        vty_out (vty, "PID[%d] Can't open configuration file %s.%s", getpid(), config_file_tmp, VTY_NEWLINE);
        goto finished;
    }
    ...
    if (config_file_sav != NULL)
    {
        ...
        if (unlink (config_file) != 0) {
            vty_out (vty, "Can't unlink configuration file %s.%s", config_file, VTY_NEWLINE);
            goto finished;
        }
    }
    if (link (config_file_tmp, config_file) != 0) {
        vty_out (vty, "Can't save configuration file %s.%s", config_file, VTY_NEWLINE);
        goto finished;
    }
    sync ();
       ...
       "Write to terminal\n")
{
    ...
    /*fprintf(stderr, "vty->type=%d%s", vty->type, VTY_NEWLINE);*/
    if (vty->type == VTY_SHELL_SERV) {
        /*fprintf(stderr, "get vty shell config.%s", VTY_NEWLINE);*/
        for (i = 0; i < vector_active (cmdvec); i++)
            if ((node = vector_slot (cmdvec, i)) && node->func) {
                /*fprintf(stderr, "node->node=%d%s", node->node, VTY_NEWLINE); // && node->vtysh*/
                if ((*node->func) (vty))
                    vty_out (vty, "!%s", VTY_NEWLINE);
            }
    }
    else {
        /*fprintf(stderr, "start.1%s", VTY_NEWLINE);*/
        vty_out (vty, "%sCurrent configuration:%s", VTY_NEWLINE, VTY_NEWLINE);
        vty_out (vty, "!%s", VTY_NEWLINE);
        /*fprintf(stderr, "start.cmdstr:%s%s", self->string, VTY_NEWLINE);*/

        for (i = 0; i < vector_active (cmdvec); i++)
            if ((node = vector_slot (cmdvec, i)) && node->func) {
                /*fprintf(stderr, "start.node=%d%s", node->node, VTY_NEWLINE);*/
                if ((*node->func) (vty))
                    vty_out (vty, "!%s", VTY_NEWLINE);
...
DEFUN (config_hostname,
       ...
       "This system's network name. <S><Length 1-16>\n")
{
    char buf[128];
    ...
    if (strlen(argv[0]) > 16) {
        vty_out (vty, "The length of the name is too long!%s", VTY_NEWLINE);
        return CMD_WARNING;
    }
    ...
    host.name = XSTRDUP (MTYPE_HOST, buf);
       ...
       "Reset system's network name\n")
{
    ...
    host.name = (char *) XSTRDUP (MTYPE_HOST, HOST_NAME_DEFAULT);
...
DEFUN (show_process_info,
       show_process_info_cmd,
       "show process info", 
       SHOW_STR 
       "Show process\n" 
       "Process information\n")
...
DEFUN (test_config_time_1,
       test_config_time_1_cmd,
       "itime YYYY/MM/DD-HH:MM:SS",
       "string imput\n"
       "Input zebra version\n")
{
    int i = 0;
...
DEFUN (test_config_time_2,
       test_config_time_2_cmd,
       "itime YYYY/MM/DD",
       "string imput\n"
       "Input zebra version\n")
{
...
DEFUN (test_config_time_3,
       test_config_time_3_cmd,
       "itime HH:MM:SS",
       "string imput\n"
       "Input zebra version\n")
{
...
DEFUN (test_config_1,
       ...
       "Input zebra version\n")
{
  vty_out (vty, "something: %s.%s", argv[0], VTY_NEWLINE);
...
DEFUN (test_config_3, 
        test_config_3_cmd,
       "test-d {aaa|ddd|c \"STRING\"|(ddd|\"STRING\")}",
       "Modify enable password parameters\n"
       "aaa\n"
       "ddd\n"
       "c\n"
...
extern int tflog_alarm_priority_set(int prio);
DEFUN (alarm_priority_severity,
       alarm_priority_severity_cmd,
...
extern int tflog_syslog_priority_set(int prio);
DEFUN (syslog_priority_severity,
       syslog_priority_severity_cmd,
...
cmd_init (int terminal)
{
    ...
    token_cr.desc = XSTRDUP (MTYPE_CMD_TOKENS, "Please press ENTER to execute command");
    ...
    host.name = NULL;
    host.name = (char *) XSTRDUP (MTYPE_VTY, HOST_NAME_DEFAULT);
    host.password = (char *) XSTRDUP (MTYPE_VTY, PASSWORD_DEFAULT);
    host.enable = NULL;
    host.logfile = NULL;
    host.config = NULL;
    host.lines = -1;
    ...
    //stephen.liu 20160719
    setCommandVersion(&command_version, CMD_VERSION_SERVICE_PORT);
    ...
    install_element (VIEW_NODE, &test_config_time_1_cmd);
    install_element (VIEW_NODE, &test_config_time_2_cmd);
    install_element (VIEW_NODE, &test_config_time_3_cmd);
#if 0    
    install_element (VIEW_NODE, &test_config_2_cmd);
    install_element (VIEW_NODE, &test_config_3_cmd);
#endif
    if (terminal) {
    ...
    if (terminal) {
        ...
        install_element (ENABLE_NODE, &show_current_cfg_cmd);
        install_element (ENABLE_NODE, &config_save_cmd);
    }
    ...
    if (terminal) {
        ...
        install_element (CONFIG_NODE, &show_current_cfg_cmd);
        install_element (CONFIG_NODE, &config_save_cmd);

        install_element (CONFIG_NODE, &alarm_priority_severity_cmd);
        install_element (CONFIG_NODE, &syslog_priority_severity_cmd);

        install_element (CONFIG_NODE, &show_process_info_cmd);
    }

    if (terminal) {
        install_element (ENABLE_NODE, &debug_mode_cmd);
        install_default (DEBUG_NODE);
        install_element (DEBUG_NODE, &show_current_cfg_cmd);
    }
