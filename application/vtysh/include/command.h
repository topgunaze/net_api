...
#define SINGLE_WORD_SIZE 512

/* command version define */
/* stephen.liu 20160719 */
typedef enum {
    CMD_VERSION_SERVICE_PORT = 0x1,
    CMD_VERSION_TRADITIONAL,
    CMD_VERSION_OTHER,
    CMD_VERSION_END
}CMD_VERSION_t;
...
struct host
{
  ...
  /* User login history, stephen.liu add, 20151013 */
  struct list *hist;
...
enum node_type
{
    USER_NODE,            /* Input user name ,stephen.liu add ,20151012*/
    ...
    TEST_NODE,           /* zebra connection node. stephen, 20150928*/
    ...
    SRV_PROFILE_NODE,
    SLA_PROFILE_NODE,
    SIPAGENT_PROFILE_NODE,
    POTS_PROFILE_NODE,
    DIGITMAP_PROFILE_NODE,
    SIPRIGHT_PROFILE_NODE,
    INTERFACE_GTF_NODE,
    ...
    CONFIG_DISABLE_DHCP_SNOOPING_NODE,		/*used for DHCP snooping*/
    ...
    SIPAGENT_PROFILE_INTERACTION_NODE,
    POTS_PROFILE_INTERACTION_NODE,
    DIGITMAP_PROFILE_INTERACTION_NODE,
    SIPRIGHT_PROFILE_INTERACTION_NODE,
...
#define INTERFACE_TF_NODE INTERFACE_GTF_NODE
...
struct cmd_element
{
  ...
  int right;            /* add by keith.gong 20151102 */
  int style;            /* add by gerhard.lao 20160707 */
};
...
#define CMD_SUCCESS_INTERACTION 11

/* Argc max counts. */
#define CMD_ARGC_MAX   40
...
#define CMD_VARIABLE(S) (((S[0]) >= 'A' && (S[0]) <= 'Z') || ((S[0]) == '<')||(S[0] == '\"'))
#define CMD_VARARG(S)   ((S[0]) == '.')
#define CMD_RANGE(S)	((S[0] == '<'))

#define CMD_STRING(S)	((S[0] == '\"'))

//1 stephen.liu add, 20151023, for support <0x0-0xffff>
#define CMD_HEX_RANGE(S)	(((S[0] == '<')&&(S[1] == '0')&&(S[2] == 'x'))||(S[0] == '<'))

#define CMD_IPV4(S)	   ((strcmp ((S), "A.B.C.D") == 0))
#define CMD_IPV4_PREFIX(S) ((strcmp ((S), "A.B.C.D/M") == 0))
#define CMD_IPV6(S)        ((strcmp ((S), "X:X::X:X") == 0))
#define CMD_IPV6_PREFIX(S) ((strcmp ((S), "X:X::X:X/M") == 0))
#define CMD_MAC(S) ((strcmp ((S), "AA:BB:CC:DD:EE:FF") == 0))
#define CMD_TIME(S) ((strcmp ((S), "YYYY/MM/DD-HH:MM:SS") == 0)||\
                                                                (strcmp ((S), "YYYY/MM/DD") == 0)||\
                                                                (strcmp ((S), "HH:MM:SS") == 0))
#define CMD_TIME_YEAR_MAX 2038
#define CMD_TIME_YEAR_MIN 1970
#define CMD_TIME_YEAR_CHECK(year) ((year <= CMD_TIME_YEAR_MAX)&&(year >= CMD_TIME_YEAR_MIN))
#define CMD_TIME_MON_CHECK(mon) ((mon <= 12)&&(mon > 0))
/* Common descriptions. */
...
#define DEBUG_STR "<Group> debugging command group\n"
...
#define __PROMPT_SEPARATOR_LENGTH__     77
#define __PROMPT_SEPARATOR_CHAR__       '-'

/* Prototypes. */
extern void install_node (struct cmd_node *, int (*) (struct vty *));
extern void install_default (enum node_type);
extern void install_element (enum node_type, struct cmd_element *);
extern void install_element_with_right(enum node_type ntype, int right, struct cmd_element *cmd);
extern void install_element_with_style(enum node_type ntype, int style, struct cmd_element *cmd);
...
extern unsigned char cmd_element_access_allowed(int nResourceAccess, int nUserAccess);
...
extern struct cmd_element show_current_cfg_cmd;
extern struct cmd_element config_save_cmd;
extern char *host_config_file (void);
extern void host_config_set (char *);

extern void print_version (const char *);
extern void vty_set_config_write(config_write s);
