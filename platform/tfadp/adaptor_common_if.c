...
/* 内存池链队列锁 */
static NET_MUTUX g_net_mp_res_lock = PTHREAD_MUTEX_INITIALIZER;
...
net_blk_pool_create(NET_BLK_POOL *pool, const NET_BLK_POOL_PARM *parm)
{
    ...
    */
    ...
    pool->status = TF_NET_BLK_POOL_VALID;
...
net_blk_pool_destroy(NET_BLK_POOL *pool)
{
    ...
    if ( pool->status != TF_NET_BLK_POOL_VALID || pool->stat.free < pool->parm.num_blks)
    {
    ...
    pool->status = TF_NET_BLK_POOL_DELETED;
...
net_blk_pool_get_next(const NET_BLK_POOL **prev)
{
    ...
    if (pool && (pool->status != TF_NET_BLK_POOL_VALID))
    {
        return MDW_NET_RC_PARAM_OUT_OF_RANGE;
    }
